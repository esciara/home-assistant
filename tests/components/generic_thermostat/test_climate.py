...
NO_PRESET_TEMPERATURE = 23
AWAY_COOLING_TEMPERATURE = 30
AWAY_HEATING_TEMPERATURE = 16
COMFORT_TEMPERATURE = 20
HOME_TEMPERATURE = 19
SLEEP_TEMPERATURE = 17
ACTIVITY_TEMPERATURE = 21

THERMOSTAT_ID = "climate.test"
TEMPERATURE_SENSOR_ID = "sensor.test"
HEATING_COOLING_UNIT_ID = "switch.test"
...
async def test_setting_up_thermostat_with_missing_config_values(
    hass: HomeAssistant,
) -> None:
    ...
    config = {
        ...
        "name": "test",
        "target_sensor": TEMPERATURE_SENSOR_ID,
    }
    with assert_setup_component(0):
        await async_setup_component(hass, "climate", {"climate": config})


async def test_setting_up_thermostat_with_valid_conf(hass: HomeAssistant) -> None:
    ...
    assert await async_setup_component(
        ...
        "climate",
        {
            "climate": {
                "platform": "generic_thermostat",
                "name": "test",
                "heater": HEATING_COOLING_UNIT_ID,
                "target_sensor": TEMPERATURE_SENSOR_ID,
            }
        },
    )
...
@pytest.fixture
async def setup_homeassistant_components(hass):
...
async def test_setting_up_thermostat_with_heater_as_input_boolean_updates_heater_state(
    hass: HomeAssistant, setup_homeassistant_components
) -> None:
    """Test heater switching input_boolean."""
    input_boolean_test_entity = "input_boolean.test"
    assert await async_setup_component(
    ...
    assert await async_setup_component(
        ...
        DOMAIN,
        {
            "climate": {
                "platform": "generic_thermostat",
                "name": "test",
                "heater": input_boolean_test_entity,
                "target_sensor": TEMPERATURE_SENSOR_ID,
    ...
    assert hass.states.get(input_boolean_test_entity).state == STATE_OFF

    _set_sensor_temperature(hass, 18)
    await hass.async_block_till_done()
    await common.async_set_temperature(hass, 23)
    await hass.async_block_till_done()

    assert hass.states.get(input_boolean_test_entity).state == STATE_ON


async def test_setting_up_thermostat_with_heater_as_switch_updates_heater_state(
    hass: HomeAssistant,
    setup_homeassistant_components,
    enable_custom_integrations: None,
) -> None:
    """Test heater switching test switch."""
    switch_platform = getattr(hass.components, "test.switch")
    switch_platform.init()
    switch_entity = switch_platform.ENTITIES[1]
    assert await async_setup_component(
        hass, switch.DOMAIN, {"switch": {"platform": "test"}}
    )
    await hass.async_block_till_done()
    switch_entity_id = switch_entity.entity_id
...
async def test_setting_up_thermostat_unique_id(
    hass: HomeAssistant,
    ...
    _set_sensor_temperature(hass, 18)
    _check_calls_on_heating_cooling_unit(hass, True)
    assert await async_setup_component(
        hass,
        DOMAIN,
        {
    ...
    entry = entity_registry.async_get(THERMOSTAT_ID)
    assert entry
    assert entry.unique_id == unique_id


def _set_sensor_temperature(hass, temperature):
    """Set up the test sensor."""
    hass.states.async_set(TEMPERATURE_SENSOR_ID, temperature)


@pytest.fixture
async def setup_thermostat_with_heating(hass):
    """Initialize components."""
    hass.config.units = METRIC_SYSTEM
    assert await async_setup_component(
        hass,
        DOMAIN,
        {
            "climate": {
                ...
                "heater": HEATING_COOLING_UNIT_ID,
                "target_sensor": TEMPERATURE_SENSOR_ID,
                "away_temp": AWAY_HEATING_TEMPERATURE,
                "sleep_temp": SLEEP_TEMPERATURE,
                "home_temp": HOME_TEMPERATURE,
                "comfort_temp": COMFORT_TEMPERATURE,
                "activity_temp": ACTIVITY_TEMPERATURE,
                "initial_hvac_mode": HVACMode.HEAT,
            }
        },
    )
...
async def test_setting_up_thermostat_without_hvac_mode_defaults_to_hvac_mode_off(
    hass: HomeAssistant,
) -> None:
    ...
    await hass.async_block_till_done()
    assert hass.states.get(THERMOSTAT_ID).state == HVACMode.OFF


async def test_adding_temperature_sensor_updates_current_temperature(
    hass: HomeAssistant,
) -> None:
    ...
    await hass.async_block_till_done()
    assert hass.states.get(THERMOSTAT_ID).attributes["current_temperature"] == 18


async def test_setting_up_thermostat_without_defaulted_attributes_uses_defaults(
    hass: HomeAssistant, setup_thermostat_with_heating
) -> None:
...
async def test_getting_hvac_modes(
    hass: HomeAssistant, setup_thermostat_with_heating
) -> None:
...
async def test_settings_all_climate_target_temperatures_sets_thermostat_target_temperature_except_for_none(
    hass: HomeAssistant, setup_thermostat_with_heating
) -> None:
...
@pytest.mark.parametrize(
    ("preset", "temperature"),
    [
        (PRESET_NONE, NO_PRESET_TEMPERATURE),
        (PRESET_AWAY, AWAY_HEATING_TEMPERATURE),
        (PRESET_COMFORT, COMFORT_TEMPERATURE),
        (PRESET_HOME, HOME_TEMPERATURE),
        (PRESET_SLEEP, SLEEP_TEMPERATURE),
        (PRESET_ACTIVITY, ACTIVITY_TEMPERATURE),
    ],
)
async def test_setting_preset_mode_sets_target_temperature(
    hass: HomeAssistant, setup_thermostat_with_heating, preset, temperature
) -> None:
...
async def test_setting_then_removing_preset_mode_sets_then_rolls_back_target_temperature(
    hass: HomeAssistant, setup_thermostat_with_heating, preset, temperature
) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        (PRESET_NONE, NO_PRESET_TEMPERATURE),
        (PRESET_AWAY, AWAY_HEATING_TEMPERATURE),
        (PRESET_COMFORT, NO_PRESET_TEMPERATURE),
        (PRESET_HOME, HOME_TEMPERATURE),
        (PRESET_SLEEP, SLEEP_TEMPERATURE),
        (PRESET_ACTIVITY, ACTIVITY_TEMPERATURE),
    ],
)
async def test_setting_twice_then_removing_preset_mode_sets_then_rolls_back_target_temperature(
    hass: HomeAssistant, setup_thermostat_with_heating, preset, temperature
) -> None:
...
async def test_setting_invalid_preset_mode_fails(
    hass: HomeAssistant, setup_thermostat_with_heating
) -> None:
...
async def test_setting_preset_mode_works_regardless_of_string_case(
    hass: HomeAssistant, setup_thermostat_with_heating
) -> None:
    """Test an invalid mode raises an error and ignore case when checking modes."""
    await common.async_set_temperature(hass, 22)
    await common.async_set_preset_mode(hass, PRESET_AWAY.upper())
    state = hass.states.get(THERMOSTAT_ID)
    assert state.attributes.get("preset_mode") == PRESET_AWAY


async def test_setting_target_temperature_to_non_int_value_does_not_change_tarter(
    hass: HomeAssistant, setup_thermostat_with_heating
) -> None:
    ...
    _set_sensor_temperature(hass, None)
    await hass.async_block_till_done()
    state = hass.states.get(THERMOSTAT_ID)
    assert state.attributes.get("current_temperature") == temperature

    _set_sensor_temperature(hass, "inf")
    await hass.async_block_till_done()
    state = hass.states.get(THERMOSTAT_ID)
    assert state.attributes.get("current_temperature") == temperature

    _set_sensor_temperature(hass, "nan")
    await hass.async_block_till_done()
    state = hass.states.get(THERMOSTAT_ID)
    assert state.attributes.get("current_temperature") == temperature


async def test_unknown_temperature_sensor_state_induces_current_temperature_is_none(
    hass: HomeAssistant,
) -> None:
...
async def test_unavailable_temperature_sensor_state_induces_current_temperature_is_none(
    hass: HomeAssistant,
) -> None:
...
async def test_setting_target_temperature_above_current_temperature_sets_heater_on(
    hass: HomeAssistant, setup_thermostat_with_heating
) -> None:
    """Test if target temperature turn heater on."""
    calls = _check_calls_on_heating_cooling_unit(hass, False)
    _set_sensor_temperature(hass, 25)
    await hass.async_block_till_done()
    await common.async_set_temperature(hass, 30)
    assert len(calls) == 1
    call = calls[0]
    ...
    assert call.data["entity_id"] == HEATING_COOLING_UNIT_ID


async def test_setting_target_temperature_below_current_temperature_sets_heater_off(
    hass: HomeAssistant, setup_thermostat_with_heating
) -> None:
    """Test if target temperature turn heater off."""
    calls = _check_calls_on_heating_cooling_unit(hass, True)
    _set_sensor_temperature(hass, 30)
    await hass.async_block_till_done()
...
async def test_setting_target_temperature_within_cold_tolerance_does_not_set_heater_off(
    hass: HomeAssistant, setup_thermostat_with_heating
) -> None:
    """Test if temperature change doesn't turn on within tolerance."""
    calls = _check_calls_on_heating_cooling_unit(hass, False)
    await common.async_set_temperature(hass, 30)
    _set_sensor_temperature(hass, 29)
    await hass.async_block_till_done()
    assert len(calls) == 0


async def test_setting_target_temperature_outside_cold_tolerance_sets_heater_off(
    hass: HomeAssistant, setup_thermostat_with_heating
) -> None:
    """Test if temperature change turn heater on outside cold tolerance."""
    calls = _check_calls_on_heating_cooling_unit(hass, False)
    await common.async_set_temperature(hass, 30)
    _set_sensor_temperature(hass, 27)
    await hass.async_block_till_done()
    assert len(calls) == 1
    call = calls[0]
    assert call.domain == HASS_DOMAIN
...
async def test_setting_target_temperature_within_hot_tolerance_does_not_set_heater_on(
    hass: HomeAssistant, setup_thermostat_with_heating
) -> None:
    """Test if temperature change doesn't turn off within tolerance."""
    calls = _check_calls_on_heating_cooling_unit(hass, True)
    await common.async_set_temperature(hass, 30)
    _set_sensor_temperature(hass, 33)
    await hass.async_block_till_done()
    assert len(calls) == 0


async def test_setting_target_temperature_outside_hot_tolerance_sets_heater_on(
    hass: HomeAssistant, setup_thermostat_with_heating
) -> None:
    """Test if temperature change turn heater off outside hot tolerance."""
    calls = _check_calls_on_heating_cooling_unit(hass, True)
    await common.async_set_temperature(hass, 30)
    _set_sensor_temperature(hass, 35)
    await hass.async_block_till_done()
    assert len(calls) == 1
    call = calls[0]
    assert call.domain == HASS_DOMAIN
...
async def test_setting_hvac_mode_to_off_sets_heater_off(
    hass: HomeAssistant, setup_thermostat_with_heating
) -> None:
...
async def test_setting_hvac_mode_to_off_prevents_heater_from_turning_on(
    hass: HomeAssistant, setup_thermostat_with_heating
) -> None:
...
async def test_setting_hvac_mode_from_off_to_heat_allows_heater_to_turn_on(
    hass: HomeAssistant, setup_thermostat_with_heating
) -> None:
...
def _check_calls_on_heating_cooling_unit(hass, is_on):
    """Set up the test switch."""
    hass.states.async_set(HEATING_COOLING_UNIT_ID, STATE_ON if is_on else STATE_OFF)
    calls = []
...
async def test_temp_change_ac_off_within_tolerance(
    ...
    calls = _check_calls_on_heating_cooling_unit(hass, True)
    await common.async_set_temperature(hass, 30)
    _set_sensor_temperature(hass, 29.8)
    await hass.async_block_till_done()
    assert len(calls) == 0
...
async def test_temp_change_ac_on_within_tolerance(
    ...
    calls = _check_calls_on_heating_cooling_unit(hass, False)
    await common.async_set_temperature(hass, 25)
    _set_sensor_temperature(hass, 25.2)
    await hass.async_block_till_done()
    assert len(calls) == 0
...
async def test_temp_change_ac_trigger_off_long_enough_3(
    ...
    calls = _check_calls_on_heating_cooling_unit(hass, False)
    await hass.async_block_till_done()
    _set_sensor_temperature(hass, 20)
    await hass.async_block_till_done()
    await common.async_set_temperature(hass, 25)
    test_time = datetime.datetime.now(dt_util.UTC)
    async_fire_time_changed(hass, test_time)
...
async def test_initial_hvac_off_force_heater_off(hass: HomeAssistant) -> None:
    ...
    # switch is on
    calls = _check_calls_on_heating_cooling_unit(hass, True)
    assert hass.states.get(HEATING_COOLING_UNIT_ID).state == STATE_ON

    _set_sensor_temperature(hass, 16)
...
async def test_restore_will_turn_off_(hass: HomeAssistant) -> None:
    ...
    _set_sensor_temperature(hass, 22)
...
async def test_restore_state_uncoherence_case(hass: HomeAssistant) -> None:
    ...
    calls = _check_calls_on_heating_cooling_unit(hass, False)
    _set_sensor_temperature(hass, 15)
